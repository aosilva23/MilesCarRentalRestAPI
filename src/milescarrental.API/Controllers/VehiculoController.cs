...
namespace milescarrental.API.Controllers
{
    ...
    public class VehiculoController : Controller
    {
        ...
        public VehiculoController(IMediator mediator, AppConfiguration appConfig, IWebHostEnvironment hostEnvironment)
        {
        ...
        [HttpPost]
        [ApiVersion("1.0")]
        [Route("api/v{version:apiVersion}/vehiculos/procesar-vehiculos")]
        [ProducesResponseType(typeof(ApiRequestResponse), (int)HttpStatusCode.OK)]
        [SwaggerOperation(
            Summary = "Administración de Vehiculos",
            Description = "Endpoint para realizar las tareas sobre la entidad Vehiculos del aplicativo\n\n" +
        ...
        public async Task<IActionResult> RegisterUser([FromBody] VehiculoDTO vehiculo)
        {
            var apiResponseList = new ApiResponseList<VehiculoDTO>();
            ApiRequestResponse apiRequestResponse = new ApiRequestResponse();

            // Validar si el modelo o VehiculoDTO es valido:
            /*if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }*/

            var list = await _mediator.Send(new VehiculoQuery(vehiculo));
